...
for(i in c(1:length(rlist))){
    ...
    s<-varying_d(numsims=numsims,numsteps=numsteps,numlocs=numlocs,p0=K,params=params,ext_thrs=ext_thrs,
                 scl=scl,model=model,disp_everywhere=F,plotteron = F,resloc=resloc,checkon=F)
...
for(r in rlist){
    ...
    params<-c(r,K)
    s<-varying_d(numsims=numsims,numsteps=numsteps,numlocs=numlocs,p0=K,params=params,ext_thrs=ext_thrs,
                 scl=scl,model=model,disp_everywhere=T,plotteron = F,resloc=resloc,checkon=F)
...
```{r sim_pennycuick, echo=F,  results="hide", cache=T, cache.extra=list(seed,a_pennycuick,b_pennycuick,rc_pennycuick,mtime("mydispersal.R"),mtime("ExtremeTailDep.R"))}
set.seed(seed)
source("mydispersal.R")
...
pdf(paste(resloc,"pennycuick_ext_risk_local_disp.pdf",sep=""),height=9,width=15)
op<-par(mfrow=c(3,4),mar=c(5.2,4.2,1.2,6.2))
...
pdf(paste(resloc,"pennycuick_ext_risk_global_disp.pdf",sep=""),height=9,width=15)
op<-par(mfrow=c(3,4),mar=c(5.2,4.2,1.2,6.2))
...
rlist<-seq(from=0.2,to=1.8,by=0.2) #rc=1
...
pdf(paste(resloc,"verhulst_ext_risk_local_disp.pdf",sep=""),height=9,width=12)
...
pdf(paste(resloc,"verhulst_ext_risk_global_disp.pdf",sep=""),height=9,width=12)
op<-par(mfrow=c(3,3),mar=c(5.2,4.2,1.2,6.2))
...
K<-50
p0<-K
rc_abrewer<-1/(K*(1-exp(-s*K)))

rlist<-sort(c(0.002,0.004,0.006,0.008,0.01,0.015,rc_abrewer,0.022,0.026))
...
pdf(paste(resloc,"abrewer_ext_risk_local_disp.pdf",sep=""),height=9,width=12)
op<-par(mfrow=c(3,3),mar=c(5.2,4.2,1.2,6.2))
...
pdf(paste(resloc,"abrewer_ext_risk_global_disp.pdf",sep=""),height=9,width=12)
op<-par(mfrow=c(3,3),mar=c(5.2,4.2,1.2,6.2))
