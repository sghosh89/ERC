...
of $10000$ simulations with the probability of computed extinction risk at each time step (for details see [`code`](https://github.com/sghosh89/ERC/blob/master/ext_risk_cop_age_str.R)).
...
$d$ that dispersed from each patch was divided up equally among all other patches. We called $d$ as dispersal
...
independent through time with same 
spatial correlations between patches (to within sampling error) for every simulation, 
but moderately (or extremely) left- and right-tail dependent between patches in
...
A three dimensional array, $n_s \times n \times n_t$, 
