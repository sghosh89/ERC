...
Fig. \ref{SM-fig_extrisk_agestr_C_SC_G_SG} represents this scenario with 4 different 
...
(e.g. C-SC in Fig. \ref{SM-fig_extrisk_agestr_C_SC_G_SG}(A,C) and
G-SG in Fig. \ref{SM-fig_extrisk_agestr_C_SC_G_SG}(B,D)) and that difference was partly 
...
Though we found left tail dependence in vital rates (induced by ECEs) caused higher extinction risk than 
the right tail dependent ones, the difference was not remarkable (Fig. \ref{SM-fig_extrisk_agestr_C_SC_G_SG}) 
unless the tail dependence was at its extreme (Fig. \ref{fig_extrisk_agestr}). This finding could be explained 
...
2. Old study (Doak1994, Ferguson1995) showed including correlation btw vital rates for PVA makes a difference, 
   what if we include TD through copula?
3. multisite age str. model with dispersal, directional change in spatio-temporal variation for vital rates.
-->
...
would be to explore the viability for the metapopulations if vital rates across sites 
showed specific tail dependence with spatially correlated extreme climatic events. 



   
<!--What if TD with negative correlation?
e.g. some events that will kill adults will allow more successful recruitment of seeds or other propagules.
-->
...
<!--future study: more complex stage or age structured model, extinction risk how propagated through a food web-->
