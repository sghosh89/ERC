...
climatic events may modify "tail associations" between values of an environmental 
variable measured in different locations, i.e., associations that 
occur between values in the left or right tails of the distributions 
of the variable that occur in the locations. 
Two positively associated variables can be principally "left-tail associated" (i.e., more 
correlated when they take low than high values) or "right-tail associated"
(more correlated when they take high than low values), even with the same
overall correlation coefficient in both cases. We tested whether the nature of 
spatial tail associations of environmental variables can influence metapopulation 
extinction risk. For instance, if low values of a variable reduce the growth 
rates of local
...
We also tested, in the context of non-spatial stage-structured matrix models, 
whether tail associations between stage-specific vital rates may
influence extinction risk. 
We showed that density dependence interacts with the
propensity for tail associations to influence metapopulation extinction risk: for 
population models
showing undercompensatory density dependence, left-tail associations in environmental 
variables typically accentuated
...
\noindent \textbf{\textit{Keywords:}} extinction risk, extreme events, tail association, density-dependent model, copula
...
<!--**DAN: You suggested we replace this with \nameref{Methods}. I do not care
one way or the other. You decide, we either do this for all such links or 
for none. Please decide yourself and then search through and make it that
way systematically across the ms. I point out though, links between pdfs will
not work.-->
...
measured in different locations. For instance, suppose a single ECE is associated
with spatially widespread, large values 
of some climatic variable. Also suppose that small values of the
variable are associated, instead, with local weather, which can differ from location
to location, rather than with ECEs. The result could
...
analysis and/or using models that are more realistic, 
...
wider examination by other researchers. Though tail associations are used
...
gamma distributions were the transformations. 
<!--***DAN: Shyamolina, you asked if I was talking about qbeta and qgamma here. 
Yes, but we do not need to say that - these are very standard functions in R. Anyone
who would be using R will know them, anyone not using R won't care that these
are the names of the functions in R. We just have to give what the mathematical
name is, which is "inverse 
cumulative distribution functions". Thanks for correcting my spelling of cumulative. 
Please delete this comment once you have read it.-->
Because these
...
largely following Fig. 7 of Ghosh \emph{et al.} (2019).
...
<!--***DAN: Shyamolina, you made a comment here in the pdf. I understand what you are saying.
What you did was mathematically equivalent to what I have written here, so it is OK we described it this way.-->
<!--***DAN: Shyamolina, in future please make comments in the Rmd, not attached to a pdf - that 
keeps everything in one place and makes it possible to reply to your comments more easily.-->
Because this was a monotonic transformation,
...
For our metapopulation simulations, for low values of $r$, left-tail
associations of environmental noises accentuated extinction risks, and right-tail associations
mitigated them; but the reverse was true for high values of $r$ in the range
$(r_{min},r_{bf})$. Thus 
...
These results help address Q2 from the \nameref{Introduction}.
...
and the $r_c$ values of Table \ref{tab_model_summary} reveals that, for the Velhurst and Austin-Brewer models,
the switch occurs at a value of $r$ less than $r_c$ for these models. For the example
...
reveals a similar reversal for the Maynard Smith, Velhurst and Austin-Brewer models; but 
it occurs at a value of $r$ close to but distinguishable from $r_c$.

# Discussion\label{Discussion}

<!--summary results:-->
Our results combined with earlier results 
...
for overcompensatory dynamics, the oposite result held, again typically, but not always: 
...
to subsequently crash, across large areas. In contrast 
to the metapopulation case, our results 
...
human-induced climate change [@ummenhofer2017extreme]. We here provide a fuller 
explanation of how such changes may modify the asymmetry of tail association of 
weather variables measured in different places, and thereby influence metapopulation
extinction risk through the mechanisms we explored in this paper. 
If $X_i(t)$ denotes the value of an environmental variable (e.g., a summer maximum 
temperature, or winter minimum temperature, spring rainfall, etc.) 
in location $i$ in year $t$,
@ummenhofer2017extreme provide a diagram of how ECEs may be increasing in frequency 
and intensity due to changes in the distribution of $X_i(t)$ such as changes
in the mean, variance, or skewness of this distribution. Although changes in 
the correlation between $X_i(t)$ and $X_j(t)$ are increasingly documented, 
...
changing. Documented changes in correlation between $X_i(t)$ and $X_j(t)$
may be due partly or wholly to changes in tail associations. If ECEs are becoming
...
influenced should be increasing. This should contribute to increased association
...
It is known that the distributions $X_i(t)$ are changing, both in
mean and variance, and possibly in other ways, depending on what
environmental variable $X$ represents.
Studying associations between the values of an environmental variable in different
locations, $X_i(t)$ and $X_j(t)$, using standard methods such as Pearson correlation
may be complicated by these changes; rank-based and related 
measures of association such as 
we have used are recommended instead.
...
appears to indicate a change in association, which is instead a change 
in marginal distributions.
Perhaps unfortunately, the Pearson correlation,
which is the most commonly used measure of association of two variables,
depends on both marginal information and association information.
It is possible that recently observed changes in environmental
...
variables, rather than reflecting true changes in association.
...
to the marginal distributions of these two variables, and the information
that pertains solely to their association. 
@Ghosh_copula argued generally for the potential of copulas and related ideas
...
which we have used are closely related to copula approaches, and @Ghosh_copula,
@Genest2007 and others have recommended copula and rank approaches when the 
goal is to understand associations between variables, and how such 
associations may be changing.

<!--Tuljapurkar approximation-->
Our results suggest that tail associations and asymmetries of tail associations
may have limited influence on the extinction risk of 
stage-structured, non-spatial models. Although we believe more research is 
certainly still needed
as to whether this conclusion holds more generally than the model we considered
(see below), if it does hold 
generally, it may be possible to obtain a better understanding of 
the result using a well-known approximation
of Tuljapurkar [@tulja1982; @tulja1990] for the long-term stochastic growth rate.
...
extintion risk estimates for density-independent age- or 
...
$M$. 
<!--***DAN: Shyamolina, I did not put it as a formula because that would have 
required a lot more text - we would have to define all the terms. We don't
actually need to know the specific dependence, just the fact that the growth
rate depends on the variances and covariances. -->
None of these quantities depends on tail associations between matrix entries.
...
asymmetric tail associations.
We also note that this reasoning pertains only to density-independent models,
and says nothing about our results for the density-dependent version of our
stage-structured model. 

<!--***DAN: decided the below was a step too far
Finally, it seems likely that the 
stochastic matrix modelling framework, and Tuljapurkar's approximation,
should apply to the density-independent metapopulation model of 
@Ghosh_copula, which was comprised of Lewontin-Cohen models linked 
by dispersal. Nevertheless asymmetry of tail associations made a 
big difference to extinction risk for that model [@Ghosh_copula].
We have not examined whether Tuljapurkar's approximation is
accurate in that spatially explicit case.

***DAN: Dan to maybe add another para on the fact that I thnk Tulja's approx should also
work for our spatial, non-dd model (check first, Dan), but we see a strong effect here,
so not clear why. And also suggests the possibilty that for some age-structured models,
tail associations will make a bigger difference then they did in our case. Maybe somehow
get to the suggestion that someone needs to revisit Tulja's approx and the analytic
approximation of extinction risk of Lande and Orzack looking at tail associations? 
...
<!--future ideas-->
In our opinion, additional research would be worthwhile on the 
impact of tail associations on growth rates and 
extinction risk under additional modelling scenarios not considered
in this paper. First, we considered a model with positive association between 
...
an upper limit may be well be associated with,
for instance, $r_2$, whereas lower values of $r_1$ may not be strongly
associated with $r_2$ because reductions/increases in $r_1$ can
then be traded off against increases/reductions in *either* 
$r_2$ or $r_3$. Second, for our metapopulation models,
we assumed habitat patches were equispaced on a transect or
...
neither of these assumptions holds. Connecting the ideas of tail association that
we have developed in this study with the ideas on the geography of synchrony
...
(A, B) and extreme (C, D) asymmetry of tail association. The solid line is the estimated extinction risk, and the area of shading, which is barely visible because it is very narrow, shows 95\% confidence intervals for extinction risk.\label{fig_extrisk_agestr}}
\end{center}
\end{figure*}
...
were very narrow, as in Fig. \ref{fig_extrisk_agestr}.\label{fig_extrisk_allmodel_localdisp}}
\end{center}
\end{figure*}
