...
DAN FAVORITE SO FAR 3) Tail associations in environmental variables and their impact on extinction risk
-->
...
***Shya: 
Figure added, spell and grammar check done for the entire Maintext, but I think I should do it again just before submission.
-->
...
successive ECEs with short time intervals between them
can weaken the resistance of plant communities.
<!--***DAN: Shyamolina, what, precisely, does it mean to "weaken the resistance of a
plant community"? Resistance to what? Please clarify.

***Shya: From @dreesen2014successive, authors concluded 
"Our results show that when experimental temperate herbaceous assemblages were exposed to an extreme climatic
event, their resistance to a subsequent event, in terms of leaf
survival, is reduced only when the recovery period is relatively short (14 days for the assemblages studied here) and
not when it is longer."-->
...
variables are more associated with their smaller values than in their larger values,
...
association, with the same overall correlation. So the tail association
...
be a stronger right-tail association between measurements of the climatic variable
...
were the same, because it that case it would be very good years for the component 
...
For Q1, we use a simple 2-stage model. We hypothesize that left-tail dependence
...
For Q2, we consider spatial extensions of several simple and commonly
used population models. Though tail associations are used
...
In all cases, Kendall's $\tau$ was the same ($\tau \approx$ `r ktau_common`),
...
\end{center}
\end{figure*}
<!--***DAN: Shyamolina, can you please put the actual Kendall tau that pertained
for each dataset on each corresponding
panel? Use, say, 3-4 digits accuracy for display. These values should be very similar, 
but are expected to differ due to sampling variation. I would like to make that
clear.
***Shya: correlation added (rounded upto 4 digit) in each plot.-->

# Methods\label{Methods}

<!--stochastic matrix model : age-structured-->
To investigate (Q1) from \nameref{Introduction} that how do vital rates with an asymmetric tail 
...
We assumed the population was semelparous, i.e., adult ($A$) went through the
...
We explored (Q2) by examining how the nature of the tail association of an
...
and the Malthus-Condorcet-Mill model (terms containing logarithm of the extinct population does not make sense). 
...
with the provision of "local" or "global" dispersal among patches. An n $\times$ n dispersal
