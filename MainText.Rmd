...
important variables and demonstrated one example of density-independent metapopulation model where 
copula was used to estimate extinction risk. Copula, a rank based measure of association, is much more informative 
than linear correlation measure and a widely applied tool in dependence 
modeling [@joe2014_dependence; @nelsen2006_copula]. We here introduce the concept of copula in a
manner that follows [@Ghosh_copula]. Copulas separate the information content of a bivariate dataset,
...
association between the variables. Here, we will extend the application of copula 
in estimating extinction risk as introduced by [@Ghosh_copula] on why the potential use of copula
...
A major benefit of the copula approach is that it can effectively capture tail dependence. 
Tail dependence, already exemplified in Fig..... (add Fig.), is associations between the more extreme 
...
values in the left or lower portions, i. e. smaller values 
(respectively, the right or upper portions, i.e. higher values) 
of the two distributions. For example, extreme cold winter during growing season
...
on the dynamics of the model. Though tail-dependence 
...
<!--Tuljapurkar estimation to be explained in Discussion-->
