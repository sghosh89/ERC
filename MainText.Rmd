...
<!--
Nature of tail dependence in environmental noise (or in vital rates) affects extinction risk of a population differently.
...
from density-independent metapopulation model (higher extinction risk if noise across patches were 
...
a generalized form : $\vec{P_{t+1}} = D \lambda_t \vec{P_t}$,
...
But they were made to exhibit right- or left-tail dependence between patches in
...
To estimate extinction risk, we started with 
initial population $P_{t=0}^i = P_e^i$ for all patches ($i = 1,2,...,n$) where 
...
This extinction threshold was set to 10\% of initial population $P_e$ for individual model. 
