...
<!--***Shya: I rephrased the following para-->
There are reasons to hypothesize that different locations when influenced by single ECEs may 
show asymmetric tail associations through time. This asymmetry arises because a single 
ECE is associated with spatially widespread, large values 
of some climatic variable whereas small values of the same
variable are associated, instead, with local weather, which can differ from location
to location, rather than with ECEs. For example, rainstorms which deposit 
exceptionally large amounts of rain may do so over large areas. But periods with 
less rain may experience scattered showers only, which may be much 
less correlated across locales. The result could
be stronger right-tail association, through time, between measurements of rainfall
...
the realism of a potential mechanism. The formulation (\ref{eq_age_str}) is quite common 
in population-demography modeling [@Caswell2000; @MorrisDoak2002] where vital rates 
are found to be time-dependent and contains stochasticity (due to random 
environmental noise) in population projection matrix. The stochastic vital 
...
and Austin-Brewer models - as adapted from @cohen1995unexpected.
...
using rank-based methods (these studies used Pearson correlations).
...
associations may be changing. We recognize that the Pearson correlation
of $X_i(t)$ and $X_j(t)$, and changes through time in that correlation, 
will still be an appropriate and important quantity to study 
for many applications. For instance, in cases for which $X_i$ has a linear 
or approximately linear influence on some aspect of the ecosystem
at location $i$, Pearson correlations of the ecological impacts at locations 
$i$ and $j$, which may be of fundamental interest, will relate to 
Pearson correlations between $X_i$ and $X_j$. But in many cases, the
ecological impact of $X_i$ and $X_j$ will be a nonlinear or unknown monotonic function of 
the values of $X_i$ and $X_j$. So studying rank-based correlations
may be more informative, ecologically as well as providing the mathematical advantages
listed above. 

<!--future ideas start here-->
Our results suggest some new lines of research on the 
impact of tail associations on growth rates and 
extinction risks. First, we considered a model with positive association between 
...
an upper limit may be strongly associated with,
...
$r_2$ or $r_3$. It is possible that tail associations in negatively 
related vital rates produce different outcomes for extinction risk
from what we have observed by studying positively associated vital rates.

<!--Tuljapurkar approximation-->
A second topic of research that our work suggests has to do with Tuljapurkar's
approximation of the long-term stochastic growth rate.
...
the result using the well-known approximation
...
asymmetric tail associations. It seems worth explicitly exploring the accuracy of 
Tuljapurkar's approximation under scenarios of tail association in the vital rates
and relating results to the effects of tail associations on the long-term
...
and population variables (known as "geography of synchrony"); @walteretal2017 explored the 
mechanisms, consequences and ecological importance of this geography of synchrony. For instance, 
synchrony was found to have a prominent geography (i.e. a distinctive spatial pattern) which can 
