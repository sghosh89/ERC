...
climate signals, and can directly contribute to changes in the symmetry or
...
At the community level, research has showed that ECEs can reorder
the dominance heirarchy of species in an area, sometimes leading to 
selective local extictions, and can also produce effects that cascade 
...
variables are more associated in their smaller values than in their larger values,
...
For example, rain storms which deposit exceptionally large amounts of rain
...
populations which would occur at the same time in many patches [@Ghosh_copulas].  
@Ghosh_copula substantiated this intuition by showing that it held true for a 
modelling setup using a spatial version of a very simple non-density dependent
...
In this paper, we will explore the potential for asymmtric tail associations to 
...
locations influence extinction risk of a density dependendent metapopulation model
...
relative to right-tail dependence, even when overall correlation is kept fixed. 
...
importance for extinction risks of populations and species (but see @Ghosh_copulas). 
