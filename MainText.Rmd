...
"left-tail" or "lower-tail" association (Figure \ref{fig_intro_cop}A, B). Similarly, if two positively correlated 
variables are more associated in their larger values than smaller, they are said to have stronger
"right-tail" or "upper-tail" association (Figure \ref{fig_intro_cop}C, D). Figure \ref{fig_intro_cop} shows 
two random variables can have a wide range of assciation from extreme to moderate degree in their extreme tails, but
with a fixed Kendall's $\tau$.  




” But this is not the topic sentence. The topic sentence comes second and is something like “Tail associations are…” or “We say two positively associated variables are more right- than left-tail associated if…”. Then the para continues, its point being to explain what asymmetric tail associations are. You can use a pedagogical figure. One thing you probably want to include is the logic (see BIVAN) that if extreme events are getting stronger and more extreme/intense events are also more spatially widespread, then it seems reasonable to expect that spatial patterns of tail associations in environmental variables will also be changing. You may also want to mention (one sentence only) that we use the statistical concept of the copula to control tail dependence in our simulations – more will be said about copulas in the Methods.  
...
We recently [@Ghosh_copula] used this tail association feature using "copula" - a statistical 
...
A major benefit of the copula approach is that it can effectively capture the tail association. 
Tail association, already exemplified in Fig..... (add Fig.), is association between the more extreme 
values of distributions. For two positively associated variables, left-tail 
(respectively, right-tail) association is stronger association between
...
of the two distributions. Stronger association in either tail causes asymmetric copula 
...
left and right tail association between variables, respectively.
...
tail association between vital rates when temperature is too cold than normal) and 
...
(Q2) How does environmental noises with asymmetric tail association across multiple patches
...
correlation ($=0.6$) but opposite association in their respective tails. 
...
the metapopulations when spatially correlated noises were left-tail dependent than the one with right-tail association.
...
in years of extreme events ("catastrophs" and "bonanzas"). Though we could expect left tail association 
...
unless the tail association was at its extreme (Fig. \ref{fig_extrisk_agestr_etd}). This finding could be explained 
...
on the linear covariance measure between any two matrix elements, not on their tail-association properties. Therefore,
following @tuljapurkar1990lecture, population growth rate in an iid environment could be approximated
as same for our model (Eq. \ref{eq_age_str}) with left-tail or right-tail association.
...
\caption{Extinction risk against time from single patch age structured $(A)$ linear ($\Omega = \infty$) and $(B)$ nonlinear ($\Omega = 100$) models for $10000$ simulations; shading represents 95\% confidence interval. Fecundity of adults and survival of eggs have either extreme left or extreme right tail association. \label{fig_extrisk_agestr_etd}}
\end{center}
\end{figure*}
