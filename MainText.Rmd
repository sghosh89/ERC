...
Some material in the Discussion outlines possible next steps.
...
different, which will mean readers wont think anything funny is going on.-->
...
<!--***DAN: Insert para here talking about in what sense the variables
$s_e(t)$ and $f_a(t)$ are asymmetrically associated, through time, in their 
tails. This is where you intro the partial Spearman and may also make references
to Genest and others indicating that, when you are interested in dependence,
you want a rank-based notion of association - it's the only thing that
separates the association from the marginals. I will insert some text now,
but it won't fit perfectly until after we go through and replace Kendall with
Spearman throughout.-->
...
are defined to have rank 1),
consider the portions of the 
lines $x+y = 2l_b$ and $x+y=2u_b$ which meet with the unit square
(Fig. \ref{PartialSpearmanFig}). 
<!--***DAN: need to make this figure-->
...
we defined (Fig. \ref{PartialSpearmanFig}) in
the unit square. The quantity $\cor_{0,0.5}(u,v)-\cor_{0.5,1}(u,v)$ 
...
related by a monotonic transformation (and likewise for 
...
<!--***DAN: Shyamolina, the below commented text makes me wonder if I am misunderstanding something, please
...
non-differentiability means we cannot use the model. Perhaps it does mean that, but I don't see 
it. Let's discuss. I'm not saying we have to go back and use it, but we may have to justify
differently why we used 6 of Cohen's models and not the other two.-->
and the Malthus-Condorcet-Mill model, which employs a logarithm of the population and is therefore
inconvenient to analyze in the context of extinction.
<!--***DAN: Unless I am mistaken, the limit of the right hand side of the M-C-M model as
described by Cohen, as P approaches 0 from the right, is 0. That means the model has
a removable singularity at 0, i.e., we could just define the model so P_t+1=0 when P_t
is 0, and we would get a perfectly valid continuous model. This means that using logs is 
not really a valid reason to not use the models. Let's discuss. I'm not saying we have to go 
back and use it, but we may have to justify differently why we used 6 of Cohen's models and not 
the other two.-->
...
between all patches and in all simulations (Kendall's $\tau$ again $0.75$, up to sampling variation), 
...
for $t=1,\ldots,T$ with $\tau=0.75$ between $u_i(t)$ and $u_j(t)$ for all $i \neq j$
(up to sampling variation), but with either extreme left-, moderate left-, moderate right-,
...
the Kendall correlation between $\epsilon_i(t)$ and $\epsilon_j(t)$ was the same 
as that between $u_i(t)$ and $u_j(t)$. The quantities $(u_1(t),\ldots,u_n(t))$
for $t=1,\ldots,T$ were generated as described in Appendix \ref{SM-makenoise}.

<!--***DAN: Dan is starting to wonder if referees will wonder something like "what 
happens to tail associations when you do these transformatons." We do not wonder 
this because we have in mind a defn for tail associations that only depends on ranks,
but we have not presented that defn. We have instead only given an intuitive description
of tail association. This may be a problem. Referees will see uf generating data with tail associatons
and uniform marginals, but then transforming it, and putting the transformed values into 
our models. And we are confortable with the idea that that transformation does not
alter the tail associations, but they are not comfortable with that.
We may need to define tail associations
using partial Spearman or something, to get this idea across. The problem with that is, 
we use Kendall throughout
the rest of the ms, and so if we define partial Spearman that will create an inconsistency
(using Kendall in some places, Spearman in others) that may raise eyebrows in a short paper 
like this. Not sure right now what to do anout this, we need to think about it. Can we add a new
para at the beginnign of methods giving some kind of formal introduction to tail associations
that makes clear it is something we consider to be for ranks? Without using partial Spearman?
There is no partial Kendall, since Kendall is based on concordant and discordant pairs.
An alterntive is to replace Kendall wth Spearman throughout the paper, then go ahead
and intro partial Spearmn as a way of making it clear tail associaton is for ranks. 
Is this possible? It would definitely be a pain, so therefore want to avoid unless
necessary. One thing this would require would be calculating the populaton (not sample) 
Spearman of the extreme tail associated data. At least we could estimate it very precisely
using a huge sample (would not have the same problems doing that as we did for Kendall),
and may even be able to calculate it analytically. Let's just use Spearman and calculate it very
precisely using a large sample. Well, maybe we could calculate it analytically. 
-->

To calculate extinction risk for a given model and a given choice of tail association,
we simulated the model for $25$ time steps, $10000$ times, using $n=5$ habitat patches.
The initial condition $P_e$ was used in each habitat patch ($P_e$ is the equilibrium value of the 
one-patch, deterministic version of the model, see Table \ref{tab_model_summary}).
...
<!--***DAN: Dan stopped here on 2019 10 27 until we resolve some of the issues that have come up-->
...
```
<!--***DAN: Shyamolina, in this table and in the sup mat (and anywhere else where this comment applies),
please replace the notation P^i_t and \epsilon^i_t by P_i(t) and \epsilon_i(t), the reason being,
putting an i in the superscript makes it look like and exponent. This is legitimately confusing because
we use lots of other exponents. I want you to generally refrain from placing the abitat patch index in the
superscript, putting in the subscript instead. This means you have to put the t in parens in front of the
variable in question, instead of in the subscript. This will require several changes to this table. Please
make those changes and then commit. Then please make the corresponding changes to the sup mat, whereever needed.-->
<!--***DAN: Dan, once Shyamolina does the above, you will then have to go back and edit this table in various
ways that you cannot do until after that change is made.-->
