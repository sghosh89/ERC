...
For past few decades, extreme climatic events (ECE) appeared more common than 
...
<!--Earlier study on metapopulation extinction risk-->
...
A recent study [@Ghosh_copula] used the tail dependence feature 
to estimate extinction risk of a density-independent 
metapopulation model. Present manuscript is an extension for the application of copula 
in estimating the extinction risk as introduced by [@Ghosh_copula]. We will show 
why the potential use of copula in modelling extinction risk goes beyond the density-independent model 
...
computed extinction risk for $n_t = 25$ years, $n = 5$ patches
and $n_s = 10000$ simulations with both moderate and extreme tail dependence (left-, right-tail).  
...
\caption{Extinction risk from single patch age structured $(A)$ linear ($\Omega = \infty$) and $(B)$ nonlinear ($\Omega = 100$) models against time (shading represents 95\% confidence interval). Fecundity of adults and survival of eggs have either extreme left or extreme right tail dependence, $n_s=10000$. \label{fig_extrisk_agestr_etd}}
\end{center}
\end{figure*}
...
among $n=5$ patches, extinction risk after $n_t = 25$ years for $n_s = 10000$ simulations were calculated.
