---
title: "Supporting Information : Asymmetric tail-association in extreme climatic events affects population-extinction risk"
...
library(abind)
source("mtime.R")
# Note : for moderate tail dep. we used Kendall's tau = 0.6 to get C or SC
```

```{r get_ktau_common, echo=F, results="hide", cache=T, cache.extra=list(seed,mtime("ExtremeTailDep.R"))}
set.seed(seed)
library(copula)
...
#for(i in 1:5){ # for loop to ensure the consistency of Kendall's tau with specified np
  xtc_r<-retd(n=np,d=2,rl=1,mn=0,sdev=1) # copula with extreme right tail

  start_time <- Sys.time()
  ktau_common<- cor(xtc_r[,1],xtc_r[,2],method="kendall") #get the kendall's tau
  end_time <- Sys.time()

#  cat("points = ",np, ", ktau_common = ", ktau_common, ", run time = ", (end_time -start_time),"\n")
#}

# see the change in 3rd decimal place
#points =  1e+05 , ktau_common =  0.7512526 , run time =  1.837576 
#points =  1e+05 , ktau_common =  0.7507594 , run time =  1.864947 
#points =  1e+05 , ktau_common =  0.7497681 , run time =  1.862879 
#points =  1e+05 , ktau_common =  0.7487744 , run time =  1.854317 
#points =  1e+05 , ktau_common =  0.7516491 , run time =  1.840355 
...
parC<-iTau(copC,tau=ktau_common)
...
legend(x="topleft",legend="B",bty="n",cex=2.5)

# moderate right tail
plot(mtc_r[1:500,1],mtc_r[1:500,2],col=rgb(0,0,0,0.1),pch=19,xlab="u",ylab="v",cex.lab=2.5,cex.axis=2)
legend(x="topleft",legend="C",bty="n",cex=2.5)

# extreme right tail
plot(xtc_r[1:500,1],xtc_r[1:500,2],col=rgb(0,0,0,0.1),pch=19,xlab="u",ylab="v",cex.lab=2.5,cex.axis=2)
legend(x="topleft",legend="D",bty="n",cex=2.5)

par(op)
dev.off()

saveRDS(ktau_common,"./Results/ktau_common.RDS")
```

```{r sim_linear_mtd, echo=F,  cache=T, cache.extra=list(seed,ktau_common,mtime("ext_risk_cop_age_str.R"))}
...
omg<-Inf
ktau<-ktau_common  # kendall's tau
...
legend(x=-2,y=1.05, c("vital rates: moderate LTdep.,C","vital rates: moderate RTdep.,SC"), lty=c(1,1),  
       col=c('red', 'blue'), horiz=F, bty='n', cex=1.5, x.intersp = 0.2, seg.len = 1)
par(op)
dev.off()
```

```{r sim_nonlinear_mtd, echo=F,  cache=T, cache.extra=list(seed,ktau_common,mtime("ext_risk_cop_age_str.R"))}
...
tempo<-paste("./Results/age_str_results/",BiCopName(family=fam),"_par_",par,"_ktau_",ktau,"_nonlinear",sep="")
if (!dir.exists(tempo)){
  dir.create(tempo)
}
...
polygon(c(rev(t), t), c(rev(x$CI0.975), x$CI0.025), col = rgb(0,0,1,0.3), border = NA)
lines(t,x$erA,col="blue",type="l")
legend(x=-2,y=1.05, c("vital rates: extreme LTdep.","vital rates: extreme RTdep."), lty=c(1,1),  
       col=c('red', 'blue'), horiz=F, bty='n', cex=1.5, x.intersp = 0.2)
par(op)
dev.off()
```
...
```{r sim_ricker_scl_1, echo=F,  cache=T, cache.extra=list(seed,ktau_common,mtime("mydispersal.R"),mtime("ExtremeTailDep.R"))}
set.seed(seed)
...
for(i in c(1:length(rlist))){
    ...
    ss<-varying_d(numsims=numsims,numsteps=numsteps,numlocs=numlocs,p0=K,params=params,ext_thrs=ext_thrs,
                 scl=scl,ktau=ktau,model=model,disp_everywhere=F,plotteron = F,resloc=resloc, getacf=F)
...
for(r in rlist){
    ...
    params<-c(r,K)
    ss<-varying_d(numsims=numsims,numsteps=numsteps,numlocs=numlocs,p0=K,params=params,ext_thrs=ext_thrs,
                 scl=scl,ktau=ktau,model=model,disp_everywhere=T,plotteron = F,resloc=resloc, getacf=F)
...
#-------------------- plot for extreme and moderate tail dep. noise ---------------------------------------
pdf(paste(resloc,"ricker_extrisk_d0_extreme_and_moderatenoise.pdf",sep=""),height=5,width=5)
op<-par(mar=c(5,4,1.5,1.5),mgp=c(2,0.7,0))
...
```{r sim_hassell_scl_1, echo=F,  cache=T, cache.extra=list(seed,ktau_common,mtime("mydispersal.R"),mtime("ExtremeTailDep.R"))}
set.seed(seed)
source("mydispersal.R")
...
#-------------------- plot for extreme and moderate tail dep. noise ---------------------------------------
pdf(paste(resloc,"hassell_extrisk_d0_extreme_and_moderatenoise.pdf",sep=""),height=5,width=5)
op<-par(mar=c(5,4,1.5,1.5),mgp=c(2,0.7,0))
...
```{r sim_msmith_scl_0.8, echo=F, cache=T, cache.extra=list(seed,ktau_common,mtime("mydispersal.R"),mtime("ExtremeTailDep.R"))}
set.seed(seed)
source("mydispersal.R")
...
#---------------------- plot for extreme and moderate tail dep. noise ---------------------------------------
pdf(paste(resloc,"msmith_ext_risk_local_disp_extreme_and_moderatenoise.pdf",sep=""),height=12,width=12)
op<-par(mfrow=c(4,4),mar=c(5,4,1.5,1.5),mgp=c(2,0.5,0))
...
pdf(paste(resloc,"msmith_ext_risk_global_disp_extreme_and_moderatenoise.pdf",sep=""),height=12,width=12)
op<-par(mfrow=c(4,4),mar=c(5,4,1.5,1.5))
...
#-------------------- plot for extreme and moderate tail dep. noise ---------------------------------------
pdf(paste(resloc,"msmith_extrisk_d0_extreme_and_moderatenoise.pdf",sep=""),height=5,width=5)
op<-par(mar=c(5,4,1.5,1.5),mgp=c(2,0.7,0))
...
```{r sim_pennycuick_scl_1, echo=F, cache=T, cache.extra=list(seed,ktau_common,a_pennycuick,b_pennycuick,rc_pennycuick,r_bf_pennycuick,mtime("mydispersal.R"),mtime("ExtremeTailDep.R"))}
set.seed(seed)
source("mydispersal.R")
...
#--------------------------- plot for extreme and moderate tail dep. noise ---------------------------------------
pdf(paste(resloc,"pennycuick_ext_risk_local_disp_extreme_and_moderatenoise.pdf",sep=""),height=12,width=12)
op<-par(mfrow=c(4,4),mar=c(5,4,1.2,1.5),mgp=c(2,0.5,0))
...
pdf(paste(resloc,"pennycuick_ext_risk_global_disp_extreme_and_moderatenoise.pdf",sep=""),height=12,width=12)
op<-par(mfrow=c(4,4),mar=c(5,4,1.5,1.5),mgp=c(2,0.5,0))
...
#-------------------- plot for extreme and moderate tail dep. noise ---------------------------------------
pdf(paste(resloc,"pennycuick_extrisk_d0_extreme_and_moderatenoise.pdf",sep=""),height=5,width=5)
op<-par(mar=c(5,4,1.5,1.5),mgp=c(2,0.7,0))
...
```{r sim_verhulst_scl_0.6, echo=F,  cache=T, cache.extra=list(seed,ktau_common,mtime("mydispersal.R"),mtime("ExtremeTailDep.R"))}
set.seed(seed)
source("mydispersal.R")
...
#------------------------- plot for extreme and moderate tail dep. noise ---------------------------------------
pdf(paste(resloc,"verhulst_ext_risk_local_disp_extreme_and_moderatenoise.pdf",sep=""),height=9,width=9)
op<-par(mfrow=c(3,3),mar=c(5,4,1.5,1.5),mgp=c(2,0.5,0))
...
pdf(paste(resloc,"verhulst_ext_risk_global_disp_extreme_and_moderatenoise.pdf",sep=""),height=9,width=9)
op<-par(mfrow=c(3,3),mar=c(5,4,1.5,1.5),mgp=c(2,0.5,0))
...
#-------------------- plot for extreme and moderate tail dep. noise ---------------------------------------
pdf(paste(resloc,"verhulst_extrisk_d0_extreme_and_moderatenoise.pdf",sep=""),height=5,width=5)
op<-par(mar=c(5,4,1.5,1.5),mgp=c(2,0.7,0))
...
```{r sim_abrewer_scl_0.6, echo=F,  cache=T, cache.extra=list(seed,ktau_common,mtime("mydispersal.R"),mtime("ExtremeTailDep.R"))}
set.seed(seed)
source("mydispersal.R")
...
#--------------------- plot for extreme and moderate tail dep. noise ---------------------------------------
pdf(paste(resloc,"abrewer_ext_risk_local_disp_extreme_and_moderatenoise.pdf",sep=""),height=9,width=9)
op<-par(mfrow=c(3,3),mar=c(5,4,1.5,1.5),mgp=c(2,0.5,0))
...
pdf(paste(resloc,"abrewer_ext_risk_global_disp_extreme_and_moderatenoise.pdf",sep=""),height=9,width=9)
op<-par(mfrow=c(3,3),mar=c(5,4,1.5,1.5),mgp=c(2,0.5,0))
...
#-------------------- plot for extreme and moderate tail dep. noise ---------------------------------------
pdf(paste(resloc,"abrewer_extrisk_d0_extreme_and_moderatenoise.pdf",sep=""),height=5,width=5)
op<-par(mar=c(5,4,1.5,1.5),mgp=c(2,0.7,0))
...
plot(1, 0, bty = "n", xaxt = "n", yaxt = "n",xlab="",ylab="",type="n")
legend("topleft", c("extreme LT","moderate LT","moderate RT","extreme RT"), lty=c(1,1,1,1), pch=c(1,1,1.1), 
       col=c('red', 'orange','skyblue','blue'), horiz=F, bty='o', cex=1.5, adj=0, x.intersp=0.2)
...
pdf("./Results/allmodel_ext_risk_global_disp_extreme_and_moderatenoise.pdf",height=12,width=9)
...
dt2<-tibble("Density dependence $(\\lambda_t^{i})$ with noise $(\\epsilon_t^{i})$" = c("$\\exp(\\epsilon_t^{i}) \\hspace{0.1 cm} \\exp[r (1-P_t^{i}/K)$]",
                          "$\\exp(\\epsilon_t^{i}) \\hspace{0.1 cm} r/(1+aP_t^{i})^b$",
                          "$\\exp(\\epsilon_t^{i}) \\hspace{0.1 cm} r/[1+(aP_t^{i})^b]$",
                          "$\\exp(\\epsilon_t^{i}) \\hspace{0.1 cm} r/[1+\\exp \\{-a(1-P_t^{i}/b)\\}]$",
                          "$\\exp(\\epsilon_t^{i}) \\hspace{0.1 cm} [1+r(1-P_t^{i}/K)]$",
                          "$\\exp(\\epsilon_t^{i}) \\hspace{0.1 cm} [1+r(K-P_t^{i})\\{1-\\exp(-sP_t^{i})\\}]$"
))
...
dt6<-tibble("$r_{bf}^{i}$" = c(r_bf_ricker,
                           r_bf_hassell,
                           r_bf_msmith,
                           r_bf_pennycuick,
                           r_bf_verhulst,
...
dt<-cbind(dt1,dt2,dt3,dt4,dt5,dt6)
...
<!--FIG:  summary: all nonlinear model results for local dispersion, moderate noise : extrisk vs. d-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=15 cm]{./Results/allmodel_ext_risk_local_disp_moderatenoise.pdf}
\caption[Analogous figure as that of Fig. \ref{MT-fig_extrisk_allmodel_localdisp_etd} in the maintext but with moderate tail association in spatially correlated noises.]{Analogous figure as that of Fig. \ref{MT-fig_extrisk_allmodel_localdisp_etd} in the maintext but with moderate tail association in spatially correlated noises.\label{fig_extrisk_allmodel_localdisp_mtd}}
\end{center}
\end{figure}

<!--FIG:  summary: all nonlinear model results for global dispersion, extreme noise : extrisk vs. d-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=15 cm]{./Results/allmodel_ext_risk_global_disp_extremenoise.pdf}
\caption[Extinction risk vs. dispersal rate for six density dependent metapopulation model with global dispersal.]{For each of six nonlinear density-dependence models presented in Table \ref{MT-tab_model_summary} with global dispersal, extinction risk varies as dispersion-rate ($d$) goes from $0$ to $0.5$. Last two columns represent extinction risk calculated for bifurcation parameter $(r)$ taken from under-compensatory and over-compensatory regime of deterministic model analog, respectively.\label{fig_extrisk_allmodel_globaldisp_etd}}
\end{center}
\end{figure}

<!--FIG:  summary: all nonlinear model results for global dispersion, moderate noise : extrisk vs. d-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=15 cm]{./Results/allmodel_ext_risk_global_disp_moderatenoise.pdf}
\caption[Analogous figure as that of Fig. \ref{fig_extrisk_allmodel_globaldisp_mtd} but with moderate tail association in spatially correlated noises.]{Analogous figure as that of Fig. \ref{fig_extrisk_allmodel_globaldisp_mtd} but with moderate tail association in spatially correlated noises.\label{fig_extrisk_allmodel_globaldisp_mtd}}
\end{center}
\end{figure}

<!--FIG: Ricker model, local dispersal, extreme noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_extreme_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/ricker_scl_1/ricker_ext_risk_local_disp_extremenoise.pdf}\\
\caption[Effect of extreme tail-dependent environmental noises on extinction risk for Ricker metapopulation model with local dispersal.]{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Ricker dynamics with local dispersal; 
standard deviation $\sigma$=1 was used to generate extreme extreme tail dependent noises for 10000 simulations. \label{fig_ricker_risk_local_disp_scl_1_etd}}
\end{center}
\end{figure}

<!--FIG : Hassell model, local dispersal, extreme noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_extreme_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/hassell_scl_1/hassell_ext_risk_local_disp_extremenoise.pdf}\\
\caption[Effect of extreme tail-dependent environmental noises on extinction risk for Hassell metapopulation model with local dispersal.]{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Hassell dynamics with local dispersal; 
standard deviation $\sigma$=1 was used to generate extreme tail dependent noises for 10000 simulations. \label{fig_hassell_risk_local_disp_scl_1_etd}}
\end{center}
\end{figure}

<!--FIG: Maynard smith model, local dispersal, extreme noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_extreme_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/msmith_scl_0.8/msmith_ext_risk_local_disp_extremenoise.pdf}\\
\caption[Effect of extreme tail-dependent environmental noises on extinction risk for Maynard smith metapopulation model with local dispersal.]{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Maynard Smith dynamics with local dispersal; 
standard deviation $\sigma$=0.8 was used to generate extreme tail dependent noises for 10000 simulations.. \label{fig_msmith_risk_local_disp_scl_0.8_etd}}
\end{center}
\end{figure}

<!--FIG: Pennycuick model, local dispersal, extreme noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_extreme_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/pennycuick_scl_1/pennycuick_ext_risk_local_disp_extremenoise.pdf}\\
\caption[Effect of extreme tail-dependent environmental noises on extinction risk for Pennycuick metapopulation model with local dispersal.]{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Pennycuick dynamics with local dispersal; 
standard deviation $\sigma$=1 was used to generate extreme tail dependent noises for 10000 simulations. \label{fig_risk_local_disp_pennycuick_r_vary_scl_1_etd}}
\end{center}
\end{figure}

<!--FIG: Verhulst model, local dispersal, extreme noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_extreme_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/verhulst_scl_0.6/verhulst_ext_risk_local_disp_extremenoise.pdf}\\
\caption[Effect of extreme tail-dependent environmental noises on extinction risk for Verhulst metapopulation model with local dispersal.]{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Verhulst dynamics with local dispersal; 
standard deviation $\sigma$=0.6 was used to generate extreme tail dependent noises for 10000 simulations. \label{fig_risk_local_disp_verhulst_r_vary_scl_0.6_etd}}
\end{center}
\end{figure}

<!--FIG: Austin-Brewer model, local dispersal, extreme noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_extreme_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/austinbrewer_scl_0.6/abrewer_ext_risk_local_disp_extremenoise.pdf}\\
\caption[Effect of extreme tail-dependent environmental noises on extinction risk for Austin-Brewer metapopulation model with local dispersal.]{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Austin-Brewer dynamics with local dispersal; 
standard deviation $\sigma$=0.6 was used to generate extreme tail dependent noises for 10000 simulations. \label{fig_risk_local_disp_abrewer_r_vary_scl_0.6_etd}}
\end{center}
\end{figure}

<!--FIG: Ricker model, local dispersal, moderate noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/ricker_scl_1/ricker_ext_risk_local_disp_moderatenoise.pdf}\\
\caption[Effect of moderate tail-dependent environmental noises on extinction risk for Ricker metapopulation model with local dispersal.]{Analogous figure as of Fig. \ref{fig_ricker_risk_local_disp_scl_1_etd} but with moderate left- and right-tail dependent noises; Moderate left tail association (or right tail association) was generated by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between any two out of $n=5$ patches. \label{fig_ricker_risk_local_disp_scl_1_mtd}}
\end{center}
\end{figure}

<!--FIG : Hassell model, local dispersal, moderate noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/hassell_scl_1/hassell_ext_risk_local_disp_moderatenoise.pdf}\\
\caption[Effect of moderate tail-dependent environmental noises on extinction risk for Hassell metapopulation model with local dispersal.]{Analogous figure as of Fig. \ref{fig_hassell_risk_local_disp_scl_1_etd} but with moderate left- and right-tail dependent noises; Moderate left tail association (or right tail association) was generated by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between any two out of $n=5$ patches. \label{fig_hassell_risk_local_disp_scl_1_mtd}}
\end{center}
\end{figure}

<!--FIG: Maynard smith model, local dispersal, moderate noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/msmith_scl_0.8/msmith_ext_risk_local_disp_moderatenoise.pdf}\\
\caption[Effect of moderate tail-dependent environmental noises on extinction risk for Maynard Smith metapopulation model with local dispersal.]{Analogous figure as of Fig. \ref{fig_msmith_risk_local_disp_scl_1_etd} but with moderate left- and right-tail dependent noises; Moderate left tail association (or right tail association) was generated by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between any two out of $n=5$ patches. \label{fig_msmith_risk_local_disp_scl_0.8_mtd}}
\end{center}
\end{figure}

<!--FIG: Pennycuick model, local dispersal, moderate noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/pennycuick_scl_1/pennycuick_ext_risk_local_disp_moderatenoise.pdf}\\
\caption[Effect of moderate tail-dependent environmental noises on extinction risk for Pennycuick metapopulation model with local dispersal.]{Analogous figure as of Fig. \ref{fig_pennycuick_risk_local_disp_scl_1_etd} but with moderate left- and right-tail dependent noises; Moderate left tail association (or right tail association) was generated by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between any two out of $n=5$ patches. \label{fig_risk_local_disp_pennycuick_r_vary_scl_1_mtd}}
\end{center}
\end{figure}

<!--FIG: Verhulst model, local dispersal, moderate noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/verhulst_scl_0.6/verhulst_ext_risk_local_disp_moderatenoise.pdf}\\
\caption[Effect of moderate tail-dependent environmental noises on extinction risk for Verhulst metapopulation model with local dispersal.]{Analogous figure as of Fig. \ref{fig_verhulst_risk_local_disp_scl_1_etd} but with moderate left- and right-tail dependent noises; Moderate left tail association (or right tail association) was generated by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between any two out of $n=5$ patches. \label{fig_risk_local_disp_verhulst_r_vary_scl_0.6_mtd}}
\end{center}
\end{figure}

<!--FIG: Austin-Brewer model, local dispersal, moderate noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/austinbrewer_scl_0.6/abrewer_ext_risk_local_disp_moderatenoise.pdf}\\
\caption[Effect of moderate tail-dependent environmental noises on extinction risk for Austin-Brewer metapopulation model with local dispersal.]{Analogous figure as of Fig. \ref{fig_abrewer_risk_local_disp_scl_1_etd} but with moderate left- and right-tail dependent noises; Moderate left tail association (or right tail association) was generated by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between any two out of $n=5$ patches. \label{fig_risk_local_disp_abrewer_r_vary_scl_0.6_mtd}}
\end{center}
\end{figure}

<!--FIG : Ricker model, global dispersal, extreme noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_extreme_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/ricker_scl_1/ricker_ext_risk_global_disp_extremenoise.pdf}\\
\caption[Effect of extreme tail-dependent environmental noises on extinction risk for Ricker metapopulation model with global dispersal.]{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Ricker dynamics with global dispersal; standard deviation $\sigma$=1 was used to generate extreme tail dependent noises for 10000 simulations. \label{fig_ricker_risk_global_disp_scl_1_etd}}
\end{center}
\end{figure}

<!--FIG: Hassell model, global dispersal, extreme noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_extreme_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/hassell_scl_1/hassell_ext_risk_global_disp_extremenoise.pdf}\\
\caption[Effect of extreme tail-dependent environmental noises on extinction risk for Hassell metapopulation model with global dispersal.]{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Hassell dynamics with global dispersal; 
standard deviation $\sigma$=1 was used to generate extreme tail dependent noises for 10000 simulations. \label{fig_hassell_risk_global_disp_scl_1_etd}}
\end{center}
\end{figure}

<!--FIG: Maynard smith model, global dispersal, extreme noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_extreme_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/msmith_scl_0.8/msmith_ext_risk_global_disp_extremenoise.pdf}\\
\caption[Effect of extreme tail-dependent environmental noises on extinction risk for Maynard smith metapopulation model with global dispersal.]{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Maynard Smith dynamics with global dispersal; 
standard deviation $\sigma$=0.8 was used to generate extreme tail dependent noises for 10000 simulations. \label{fig_msmith_risk_global_disp_scl_0.8_etd}}
\end{center}
\end{figure}

<!--FIG: Pennycuick model, global dispersal, extreme noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_extreme_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/pennycuick_scl_1/pennycuick_ext_risk_global_disp_extremenoise.pdf}\\
\caption[Effect of extreme tail-dependent environmental noises on extinction risk for Pennycuick metapopulation model with global dispersal.]{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Pennycuick dynamics with global dispersal; 
standard deviation $\sigma$=1 was used to generate extreme tail dependent noises for 10000 simulations. \label{fig_risk_global_disp_pennycuick_r_vary_scl_1_etd}}
\end{center}
\end{figure}

<!--FIG: Verhulst model, global dispersal, extreme noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_extreme_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/verhulst_scl_0.6/verhulst_ext_risk_global_disp_extremenoise.pdf}\\
\caption[Effect of extreme tail-dependent environmental noises on extinction risk for Verhulst metapopulation model with global dispersal.]{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Verhulst dynamics with global dispersal; 
standard deviation $\sigma$=0.6 was used to generate extreme tail dependent noises for 10000 simulations. \label{fig_risk_global_disp_verhulst_r_vary_scl_0.6_etd}}
\end{center}
\end{figure}
...
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_extreme_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/austinbrewer_scl_0.6/abrewer_ext_risk_global_disp_extremenoise.pdf}\\
\caption[Effect of extreme tail-dependent environmental noises on extinction risk for Austin-Brewer metapopulation model with global dispersal.]{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Austin-Brewer dynamics with global dispersal; 
standard deviation $\sigma$=0.6 was used to generate extreme tail dependent noises for 10000 simulations. \label{fig_risk_global_disp_abrewer_r_vary_scl_0.6_etd}}
\end{center}
\end{figure}


<!--FIG: Ricker model, global dispersal, moderate noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/ricker_scl_1/ricker_ext_risk_global_disp_moderatenoise.pdf}\\
\caption[Effect of moderate tail-dependent environmental noises on extinction risk for Ricker metapopulation model with global dispersal.]{Analogous figure as of Fig. \ref{fig_ricker_risk_global_disp_scl_1_etd} but with moderate left- and right-tail dependent noises; Moderate left tail association (or right tail association) was generated by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between any two out of $n=5$ patches. \label{fig_ricker_risk_global_disp_scl_1_mtd}}
\end{center}
\end{figure}

<!--FIG : Hassell model, global dispersal, moderate noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/hassell_scl_1/hassell_ext_risk_global_disp_moderatenoise.pdf}\\
\caption[Effect of moderate tail-dependent environmental noises on extinction risk for Hassell metapopulation model with global dispersal.]{Analogous figure as of Fig. \ref{fig_hassell_risk_global_disp_scl_1_etd} but with moderate left- and right-tail dependent noises; Moderate left tail association (or right tail association) was generated by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between any two out of $n=5$ patches. \label{fig_hassell_risk_global_disp_scl_1_mtd}}
\end{center}
\end{figure}

<!--FIG: Maynard smith model, global dispersal, moderate noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/msmith_scl_0.8/msmith_ext_risk_global_disp_moderatenoise.pdf}\\
\caption[Effect of moderate tail-dependent environmental noises on extinction risk for Maynard Smith metapopulation model with global dispersal.]{Analogous figure as of Fig. \ref{fig_msmith_risk_global_disp_scl_1_etd} but with moderate left- and right-tail dependent noises; Moderate left tail association (or right tail association) was generated by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between any two out of $n=5$ patches. \label{fig_msmith_risk_global_disp_scl_0.8_mtd}}
\end{center}
\end{figure}

<!--FIG: Pennycuick model, global dispersal, moderate noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/pennycuick_scl_1/pennycuick_ext_risk_global_disp_moderatenoise.pdf}\\
\caption[Effect of moderate tail-dependent environmental noises on extinction risk for Pennycuick metapopulation model with global dispersal.]{Analogous figure as of Fig. \ref{fig_pennycuick_risk_global_disp_scl_1_etd} but with moderate left- and right-tail dependent noises; Moderate left tail association (or right tail association) was generated by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between any two out of $n=5$ patches. \label{fig_risk_global_disp_pennycuick_r_vary_scl_1_mtd}}
\end{center}
\end{figure}

<!--FIG: Verhulst model, global dispersal, moderate noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/verhulst_scl_0.6/verhulst_ext_risk_global_disp_moderatenoise.pdf}\\
\caption[Effect of moderate tail-dependent environmental noises on extinction risk for Verhulst metapopulation model with global dispersal.]{Analogous figure as of Fig. \ref{fig_verhulst_risk_global_disp_scl_1_etd} but with moderate left- and right-tail dependent noises; Moderate left tail association (or right tail association) was generated by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between any two out of $n=5$ patches. \label{fig_risk_global_disp_verhulst_r_vary_scl_0.6_mtd}}
\end{center}
\end{figure}

<!--FIG: Austin-Brewer model, global dispersal, moderate noise-->
\begin{figure}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=12 cm]{./Results/austinbrewer_scl_0.6/abrewer_ext_risk_global_disp_moderatenoise.pdf}\\
\caption[Effect of moderate tail-dependent environmental noises on extinction risk for Austin-Brewer metapopulation model with global dispersal.]{Analogous figure as of Fig. \ref{fig_abrewer_risk_global_disp_scl_1_etd} but with moderate left- and right-tail dependent noises; Moderate left tail association (or right tail association) was generated by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between any two out of $n=5$ patches. \label{fig_risk_global_disp_abrewer_r_vary_scl_0.6_mtd}}
\end{center}
\end{figure}


<!--FIG : all nonlinear model results with moderate noise, d=0, extrisk vs. r-->
\begin{figure*}[!h]
\begin{center}
\includegraphics[width=10 cm]{./Results/common_legend_moderate_noise.pdf}\\
\includegraphics[width=5 cm]{./Results/ricker_scl_1/ricker_extrisk_d0_moderatenoise.pdf}
\includegraphics[width=5 cm]{./Results/hassell_scl_1/hassell_extrisk_d0_moderatenoise.pdf}\\
\vspace{-0.6 cm}
\includegraphics[width=5 cm]{./Results/msmith_scl_0.8/msmith_extrisk_d0_moderatenoise.pdf}
\includegraphics[width=5 cm]{./Results/pennycuick_scl_1/pennycuick_extrisk_d0_moderatenoise.pdf}\\
\vspace{-0.6 cm}
\includegraphics[width=5 cm]{./Results/verhulst_scl_0.6/verhulst_extrisk_d0_moderatenoise.pdf}
\includegraphics[width=5 cm]{./Results/austinbrewer_scl_0.6/abrewer_extrisk_d0_moderatenoise.pdf}\\
\caption[Analogous figure as that of Fig. \ref{MT-fig_extrisk_d0_etd} but with moderarte 
tail dependent noises.]{Analogous figure as that of Fig. \ref{MT-fig_extrisk_d0_etd} but with moderarte 
tail dependent noises; Moderate left tail association (or right tail association) was generated 
by Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6 between
any two out of $n=5$ patches.\label{fig_extrisk_d0_mtd}}
\end{center}
\end{figure*}

# Generating matrix with extreme tail association  \label{extr_tail_dep}
...
equation which is frequently used in population ecology model. Let's say, $P_{t+1} = F(P_t,r)$
...
(i.e. $P_{t+1} = P_t$). Given a tiny perturbation from its equilibrium $P_e$, if the system 
returns to its original state, then $P_e$ is called to be a stable equilibrium, otherwise unstable.
Mathematically, from standard linear stability analysis we can say an equilibrium is locally 
asymptotically stable if $| {F}' | < 1$, where ${F}' = \left( \frac{\mathrm{d}F }{\mathrm{d} P} \right) _{P_t=P_e}$.
...
is represented as 
$P_{t+1}=F(P_t,r,K)=P_t \exp[r(1-P_t/K)]$, where $P$ is the population density, $r>0$ is the intrinsic 
...
can be represented as $P_{t+1} = F(P_t,r,a,b)=rP_t/(1+aP_t)^b$, where $r>0$ is the intrinsic growth rate 
...
$P_{t+1}=F(P_t,r,a,b)=rP_t/[1+(aP_t)^b]$. $r$ is the growth rate, $a, b$ are the parameters with positive values.
...
In general, model dynamics is expressed as $P_{t+1}=F(P_t,r,a,b)=rP_t/[1+\exp \left\{-a(1-P_t/b)\right\}]$, where 
...
as $P_{t+1} = F(P_t,r,K) =P_t[1+r(1-P_t/K)]$, wirh $r>0$ and $K>0$ being growth rate and carrying capacity, respectively. 
...
discrete dynamics : $P_{t+1} = F(P_t,r,K,s)= P_t[1+r(K-P_t) \left\{ 1-\exp (-sP_t)\right\}]$, where
