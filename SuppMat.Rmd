...
dt2<-tibble(Equations = c("$P_{t+1} = \\exp(\\epsilon_t) \\hspace{0.1 cm} P_t \\exp[r (1-P_t/K)$]",
                          "$P_{t+1} = \\exp(\\epsilon_t) \\hspace{0.1 cm} rP_t/(1+aP_t)^b$",
                          "$P_{t+1} = \\exp(\\epsilon_t) \\hspace{0.1 cm} rP_t/[1+(aP_t)^b]$",
                          "$P_{t+1} = \\exp(\\epsilon_t) \\hspace{0.1 cm} rP_t/[1+\\exp \\{-a(1-P_t/b)\\}]$",
                          "$P_{t+1} = \\exp(\\epsilon_t) \\hspace{0.1 cm} P_t [1+r(1-P_t)]$",
                          "$P_{t+1} = \\exp(\\epsilon_t) \\hspace{0.1 cm} P_t [1+r(K-P_t)\\{1-\\exp(-sP_t)\\}]$"
))
...
(see `iTau` function from `copula` package). For fecundity of adults, we transformed the first column of matrix 
$M$ as gamma ($\Gamma(k,\theta)$) distributed, $k = 2$ and $\theta = 1$ were shape and scale parameters, 
respectively. For a given $\theta$, increasing 
...
remain unchanged. For survival rates of eggs, we took the second column of $M$ and applied a 
...
of the survival rates of eggs. Entries within each clumn of matrix $M$, were thus independent of each others but 
the two columns of $M$ had a specified Kendall's correlation ($\tau = 0.6$) and underlying tail-dependence. 
For each $25$ timesteps from each of $10000$ simulations, the vital rates (fecundity and survival rates) 
of the stochastic projrction matrix in model (\ref{MT-eq_age_str}) were drawn pairwise from each row of matrix $M$.

For extreme tail-dependence algorithm, 
