...
\ref{fig_intro_cop}A) was generated using an algorithm originally presented 
...
\ref{fig_intro_cop}D), take the negative of the entire matrix. For
both types of noise, every entry was transformed by the cummulative distribution
...
noise was $0.875$, up to sampling variation.
<!--***DAN: fill this in-->

<!--moderate tail association noise-->
Bivarite environmental noise with moderate left-tail association (as in Fig. 
\ref{fig_intro_cop}B) was generated using a 2-dimensional Clayton copula. The statistical
...
audiences were provided by sections within the papers of @Anderson2018 and  
@Ghosh_copula. A 2-dimensional Clayton copula is equivalent to a particular bivariate 
probability density function with marginal distributions which are 
uniform on the interval $(0,1)$. The points on Fig. 
\ref{fig_intro_cop}B were independent draws from the associated random variable.
...
chosen so that the Spearman correlation was $0.875$, the same as that of the
...
is stable when $r<r_{bf}$ and unstable beyond than bifurcation point, where 
