---
title: "Appendix: Tail associations in ecological variables and their impact on extinction risk"
...
legend(x=-2,y=1.05, c("moderate LT","moderate RT", 
                      #expression(paste(Omega,"%->%", infinity))
                      expression(symbol('\127') *symbol('\256')* symbol('\245'))
                      ), 
       lty=c(1,1, NA),  
...
polygon(c(rev(t), t), c(rev(x$CI0.975), x$CI0.025), col = rgb(0,0,1,0.3), border = NA)
lines(t,x$erA,col="blue",type="l")
legend(x=-2,y=1.05, c("extreme LT","extreme RT", expression(symbol('\127') *symbol('\256')* symbol('\245'))), 
       lty=c(1,1, NA),  
       col=c('red', 'blue', NA), horiz=F, bty='n', cex=1.5, x.intersp = 0.2
       #, seg.len = 1
       ) 
...
dt2<-tibble("$\\lambda_i(t)$" = c("$\\exp(\\varepsilon_i(t)) \\hspace{0.1 cm} \\exp[r (1-P_i(t)/K)$]",
                          "$\\exp(\\varepsilon_i(t)) \\hspace{0.1 cm} r/(1+aP_i(t))^b$",
                          "$\\exp(\\varepsilon_i(t)) \\hspace{0.1 cm} r/[1+(aP_i(t))^b]$",
                          "$\\exp(\\varepsilon_i(t)) \\hspace{0.1 cm} r/[1+\\exp \\{-a(1-P_i(t)/b)\\}]$",
                          "$\\exp(\\varepsilon_i(t)) \\hspace{0.1 cm} [1+r(1-P_i(t)/K)]$",
                          "$\\exp(\\varepsilon_i(t)) \\hspace{0.1 cm} [1+r(K-P_i(t))\\{1-\\exp(-sP_i(t))\\}]$"
))
...
dt6<-tibble("$r_\\text{bf}$" = c(r_bf_ricker,
                           r_bf_hassell,
                           r_bf_msmith,
                           r_bf_pennycuick,
                           r_bf_verhulst,
...
\caption[Extinction risk v. tail association, $d$ and $r$: metapopulations, local dispersal, Ricker]{Analogous to Fig. \ref{MT-fig_extrisk_allmodel_localdisp} in the main text, but considering more values of $r$ and only the Ricker model. Local dispersal.\label{fig_ricker_risk_local_disp_scl_1}}
\end{center}
\end{figure}
...
\caption[Extinction risk v. tail association, $d$ and $r$: metapopulations, local dispersal, Hassell]{Analogous to Fig. \ref{MT-fig_extrisk_allmodel_localdisp} in the main text, but considering more values of $r$ and only the Hassell model. Local dispersal.\label{fig_hassell_risk_local_disp_scl_1}}
\end{center}
\end{figure}
...
\caption[Extinction risk v. tail association, $d$ and $r$: metapopulations, local dispersal, Maynard Smith]{Analogous to Fig. \ref{MT-fig_extrisk_allmodel_localdisp} in the main text, but considering more values of $r$ and only the Maynard Smith model. Local dispersal.\label{fig_msmith_risk_local_disp_scl_0.8}}
\end{center}
\end{figure}
...
\caption[Extinction risk v. tail association, $d$ and $r$: metapopulations, local dispersal, Pennycuick]{Analogous to Fig. \ref{MT-fig_extrisk_allmodel_localdisp} in the main text, but considering more values of $r$ and only the Pennycuick model. Local dispersal.\label{fig_risk_local_disp_pennycuick_r_vary_scl_1}}
\end{center}
\end{figure}
...
\caption[Extinction risk v. tail association, $d$ and $r$: metapopulations, local dispersal, Verhulst]{Analogous to Fig. \ref{MT-fig_extrisk_allmodel_localdisp} in the main text, but considering more values of $r$ and only the Verhulst model. Local dispersal.\label{fig_risk_local_disp_verhulst_r_vary_scl_0.6}}
\end{center}
\end{figure}
...
\caption[Extinction risk v. tail association, $d$ and $r$: metapopulations, local dispersal, Austin-Brewer]{Analogous to Fig. \ref{MT-fig_extrisk_allmodel_localdisp} in the main text, but considering more values of $r$ and only the Austin-Brewer model. Local dispersal.\label{fig_risk_local_disp_abrewer_r_vary_scl_0.6}}
\end{center}
\end{figure}
...
\caption[Extinction risk v. tail association, $d$ and $r$: metapopulations, global dispersal, Verhulst]{Analogous to Fig. \ref{fig_risk_local_disp_verhulst_r_vary_scl_0.6} but for global dispersal between habitat patches. \label{fig_risk_global_disp_verhulst_r_vary_scl_0.6}}
\end{center}
\end{figure}
...
Bivariate environmental noise with extreme left-tail association (as in Fig. 
...
noise was `r srho_common`, up to sampling variation. A derivation
of this fact is presented below.

<!--moderate tail association noise-->
Bivariate environmental noise with moderate left-tail association (as in Fig. \ref{MT-fig_intro_cop}B) was 
...
uniform on the interval $(0,1)$. The points in Fig. 
...
extreme left- and right-tail associated noise. This was accomplished with the `iRho`
...
survival Clayton copula. The survival Clayton copula is a rotation
of the Clayton copula by 180 degrees.

<!--Multivariate extreme tail association-->
Multivariate noise with extreme left-tail association was generated using a
very similar algorithm to the bivariate case,
...
<!--derivation of the fact that Spearman is 0.875 for the extreme tail dependent noise-->
We now present a derivation of the fact that, up to sampling variance,
the Spearman correlation of our extreme tail associated noise was 
$0.875$. Let $x_i$, $y_i$ be the ranks of a sample of noise, $i=1,\ldots,N$.
Assuming that $N$ is divisible by $4$ makes the presentation of the 
derivation simpler, so we adopt that assumption. We show the derivation for the left-tail
associated noise, the right-tail case being similar. Then we have
\vspace{-10pt}
\begin{equation}
\rho=\frac{\sum_{i=1}^N (x_i - \mean(x_i))(y_i-\mean(y_i))}{(N-1)\sqrt{\var(x_i)\var(y_i)}},
\end{equation}
\noindent where $\var$, here and throughout, is the sample variance, with Bessel's correction 
(denominator $N-1$). However, it is straightforward to see that $\mean(x_i) = \mean(y_i)=(N+1)/2$,
which we henceforth call $\mu$. It is also straightforward to see
that $\var(x_i)=\var(y_i)=\frac{\sum_{i=1}^N (i-\mu)^2}{N-1}$, since 
the set consisting of the $x_i$ is the same as the set consisting of the 
$y_i$, and is also the same as the set consisting of the numbers $1,\ldots,N$. 
Therefore,
\vspace{-10pt}
\begin{equation}
\rho=\frac{\sum_{i=1}^N (x_i-\mu)(y_i-\mu)}{\sum_{i=1}^N (i-\mu)^2}.
\end{equation}
\noindent We can reindex the $x_i$ and $y_i$ so that $x_i=y_i=i$ for $i=1,\ldots,N/2$.
This uses the fact that we are working with the extreme left-tail associated noise.
Then we have 
\vspace{-20pt}
\begin{align}
\rho &= \frac{\sum_{i=1}^{N/2} (i-\mu)^2 +\sum_{i=N/2+1}^N (x_i-\mu)(y_i-\mu)}{\sum_{i=1}^N (i-\mu)^2} \\
&= \frac{1}{2}+\frac{\sum_{i=N/2+1}^N (x_i-\mu)(y_i-\mu)}{\sum_{i=1}^N (i-\mu)^2}.\label{eq:rho1}
\end{align}
\noindent We define $\mu_2 = \mean_{i=N/2+1}^N(x_i)=\mean_{i=N/2+1}^N(i)$. It is straightforward
to show that this is $3N/4+1/2$. Then the numerator of the right term in 
(\ref{eq:rho1}) can be rearranged as
\vspace{-20pt}
\begin{align}
\sum_{i=N/2+1}^N (x_i-\mu)(y_i-\mu) &= \sum_{i=N/2+1}^N (x_i-\mu_2+\mu_2-\mu)(y_i-\mu_2+\mu_2-\mu) \\
&= \sum_{i=N/2+1}^N \left( (x_i-\mu_2)(y_i-\mu_2)+(x_i-\mu_2)(\mu_2-\mu)
  +(y_i-\mu_2)(\mu_2-\mu)+(\mu_2-\mu)^2 \right) \\
&= \sum_{i=N/2+1}^N (x_i-\mu_2)(y_i-\mu_2) + \frac{N}{2}(\mu_2-\mu)^2 \\
&= \sum_{i=N/2+1}^N (x_i-\mu_2)(y_i-\mu_2) + \frac{N^3}{32}.
\end{align}
\noindent The denominator of the right term in (\ref{eq:rho1}) can be rearranged as
\vspace{-20pt}
\begin{align}
\sum_{i=1}^N (i-\mu)^2 &= \sum_{i=1}^N (i^2-2\mu i +\mu^2) \\
&= \sum_{i=1}^N i^2 - 2\mu\sum_{i=1}^N i +N\mu^2.
\end{align}
\noindent Applying Faulhaber's formula to simplify the first term, this is
\vspace{-20pt}
\begin{align}
\sum_{i=1}^N (i-\mu)^2 &= \frac{N(N+1)(2N+1)}{6}-2\mu\frac{N(N+1)}{2}+N\mu^2 \label{eq:rho2}\\
&= \frac{N(N+1)(N-1)}{12},\label{eq:rho3}
\end{align}
\noindent where we have omitted the simple algebra leading from (\ref{eq:rho2}) 
to (\ref{eq:rho3}). Plugging in our expressions for the numerator and denominator,
(\ref{eq:rho1}) becomes
\vspace{-20pt}
\begin{align}
\rho &= \frac{1}{2}+\frac{\sum_{i=N/2+1}^N (x_i-\mu)(y_i-\mu)}{\sum_{i=1}^N (i-\mu)^2} \\
&= \frac{1}{2}+\frac{\sum_{i=N/2+1}^N (x_i-\mu_2)(y_i-\mu_2) + \frac{N^3}{32}}{\frac{N(N+1)(N-1)}{12}} \\
&= \frac{1}{2}+\frac{12\sum_{i=N/2+1}^N (x_i-\mu_2)(y_i-\mu_2)}{N(N+1)(N-1)}+
\frac{3N^3}{8N(N+1)(N-1)} \label{eq:rho4}
\end{align}
\noindent The second term of (\ref{eq:rho4}) is proportional to the covariance 
$\cov_{i=N/2+1}^N (x_i,y_i)$ (again using $N-1$ in the denominator, here and throughout). 
To see this, note that
\vspace{-20pt}
\begin{align}
\cov_{i=N/2+1}^N (x_i,y_i) &= \frac{\sum_{i=N/2+1}^N(x_i-\mu_2)(y_i-\mu_2)}{(N/2-1)
\frac{\sum_{i=N/2+1}^N (i-\mu_2)^2}{N/2-1}} \\
&= \frac{\sum_{i=N/2+1}^N(x_i-\mu_2)(y_i-\mu_2)}{\sum_{i=N/2+1}^N (i-\mu_2)^2}.\label{eq:rho5}
\end{align}
\noindent But it is straightforward to show that
\vspace{-10pt}
\begin{equation}
\sum_{i=N/2+1}^N (i-\mu_2)^2 = \frac{N(N+2)(N-2)}{96}.
\end{equation}
\noindent Plugging this into (\ref{eq:rho5}), we get
\vspace{-20pt}
\begin{align}
\cov_{i=N/2+1}^N (x_i,y_i) &= \frac{96\sum_{i=N/2+1}^N(x_i-\mu_2)(y_i-\mu_2)}{N(N+2)(N-2)}.
\end{align}
\noindent Therefore the middle term of (\ref{eq:rho4}) equals
\vspace{-10pt}
\begin{equation}
\frac{12\sum_{i=N/2+1}^N (x_i-\mu_2)(y_i-\mu_2)}{N(N+1)(N-1)} = \cov_{i=N/2+1}^N (x_i,y_i) \left( \frac{N(N+2)(N-2)}{8N(N+1)(N-1)} \right).
\end{equation}
\noindent The limit of this as $N \rightarrow \infty$ is $0$ because the covariance
goes to 0. Thus the limit of (\ref{eq:rho4}) as $N \rightarrow \infty$
is $7/8=0.875$, as originally claimed.
...
The equilibrium $P_e = 0$ is unstable, since ${F}' = \exp(r)$ for that equilibrium, and 
...
if $1<r<2$. Thus $r_\text{min}=0$, $r_c=1$, and $r_\text{bf}=2$.
...
intraspecific competition. The model
...
and overcompensatory dynamics if $[b/(b-1)]^b<r<[b/(b-2)]^b$, so $r_\text{min}=1$, 
$r_c = [b/(b-1)]^b$, and $r_\text{bf}=[b/(b-2)]^b$.
...
For a positive equilibrium value of $P^\star$, $r$ must be greater than $1$, so $r_\text{min}=1$. 
Then $P^\star$ will 
...
$P^\star$ will become unstable for $r>r_\text{bf}=b/(b-2)$. 
...
$r$ must be greater than $r_\text{min} = 1+\exp(-a)$. The transition point $r_c$ (from under- to 
overcompensatory dynamics) and the bifurcation point $r_\text{bf}$ were obtained
by numerical solution of ${F}'=0$ and ${F}'=-1$ at $P_e=P^{\star}$, respectively.

**The Verhulst model.** The Verhulst model 
...
point $r_\text{bf}=2$. The model shows undercompensatory dynamics for $0<r<r_c=1$
and overcompensatory dynamics for $r_c<r<r_\text{bf}$.
...
is stable when $r<r_\text{bf}$ and unstable beyond the bifurcation point, where 
$r_\text{bf} = \frac{2}{K[1-\exp (-sK)]}$. The model shows undercompensatory dynamics when
$0<r<r_c$, where $r_c = \frac{1}{K[1-\exp (-sK)]}$. It shows 
overcompensatory dynamics when $r_c<r<r_\text{bf}$.
