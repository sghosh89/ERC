---
title: "Supporting Information : Impact of asymmetric tail-dependence in extreme climatic events on extinction risk in population ecology"
...
legend("bottomright", c("Ricker",paste0("K = ",K),paste0("sd =",scl)), horiz=F, bty='n',cex=1.8)
#legend("bottomright", c("noise : LTdep.","noise : RTdep."), lty=c(1,1), pch=c(1,1), col=c('red', 'blue'), horiz=F, bty='n',cex=1.5)
par(op)
dev.off()
```
...
legend("bottomright", c("Maynard Smith",paste0("a = ",a," ,b = ",b),paste0("sd =",scl)), horiz=F, bty='n',cex=1.8)
par(op)
dev.off()
```
...
legend("bottomright", c("Verhulst",paste0("sd =",scl)), horiz=F, bty='n',cex=1.8)
par(op)
dev.off()
```
...
mtext(paste("(a = 0.5, b = 100, sd = 1)"), side = 3, line=-8.2, adj=0.6, cex=1, col="black")
x<-res_hassell_local
...
mtext(paste("(a = 0.5, b = 4, sd = 0.8)"), side = 3, line=-8.2, adj=0.6, cex=1, col="black")
x<-res_msmith_local
...
mtext(paste("(a = 0.1, b = 0.1, sd = 1)"), side = 3, line=-8.2, adj=0.6, cex=1, col="black")
...
mtext(paste("(sd = 0.6)"), side = 3, line=-8.2, adj=0.6, cex=1, col="black")
...
mtext(paste("(s = 0.2, K = 50, sd = 0.6)"), side = 3, line=-8.2, adj=0.6, cex=1, col="black")
x<-res_abrewer_local
...
dt1 <- tibble("Models $(P_{t+1}^{i} = \\lambda_t^{i} P_t^{i})$"= c("Ricker",
           "Hassell",
           "Maynard Smith",
           "Pennycuick",
           "Verhulst",
...
dt2<-tibble("Density dependence $(\\lambda_t^{i})$ with noise $(\\epsilon_t^{i})$" = c("$\\exp(\\epsilon_t^{i}) \\hspace{0.1 cm} \\exp[r (1-P_t^{i}/K)$]",
                          ...
                          "$\\exp(\\epsilon_t^{i}) \\hspace{0.1 cm} [1+r(1-P_t^{i})]$",
                          "$\\exp(\\epsilon_t^{i}) \\hspace{0.1 cm} [1+r(K-P_t^{i})\\{1-\\exp(-sP_t^{i})\\}]$"
))

dt3<-tibble("\\hspace{-0.1 cm} Parameters used" = c("K = 50, sd = 1",
                            "a = 0.5, b = 100, sd = 1",
                            "a = 0.5, b = 4, sd = 0.8",
                           "a = 0.1, b= 0.1, sd = 1",
                           "sd = 0.6",
...
<!--
# Vital rates in a stochastic age-structured model having specific tail-dependence \label{age_str_vitalrates}
A $N \times 2$ matrix, M, was generated for each of the left (Clayton, Survival Gumbel) and right 
(Survival Clayton, Gumbel) tail-dependent bivariate copulas, where $N = numsteps(=25) \times numsims(=10000)$.
We kept the same Kendall's correlation ($\tau = 0.6$) for each bivariate copula which used parameter accordingly
(see `iTau` function from `copula` package).-->
...
distribution with a specified mean ($m$) and standard deviation $(sd)$.
...
equation which is frequently found in population ecology model. Let's say, $P_{t+1} = F(P_t)$
represents the model dynamics of a single population $P$ which progress recursively via 
density-dependence function $F$ with discrete time variable $t$. Fixed-point analysis can
...
**Ricker model** [@ricker1954stock], originally introduced in the context of modelling
...
is represented as 
$P_{t+1}=P_t \exp[r(1-P_t/K)]$, where $P$ is the population density, $r>0$ is the intrinsic 
growth rate and $K>0$ is the carrying capacity. There are two equilibrium states ($P_e = 0, K$).
The equilibrium at ($P_e = 0$) is unstable as ${F}' = \exp(r) > 1$ and considered 
...
$r>2$, system shows sustained oscillation and the equilibrium becomes unstable. We called $r=1$ 
...
**Hassell model** [@hassell1975density], motivated for modelling insect populations,
can be represented as $P_{t+1} = rP_t/(1+aP_t)^b$, where $r>0$ is the intrinsic growth rate 
...
\caption{Effect of extreme tail-dependent environmental noises on extinction risk (calculated after 25 years)
for $5$ metapopulation patches which followed Ricker dynamics with local dispersal; 
numsims=10000, sd=1. \label{fig_ricker_risk_local_disp_scl_1}}
\end{center}
\end{figure}
...
numsims=10000, sd=1. \label{fig_ricker_risk_global_disp_scl_1}}
\end{center}
\end{figure}
...
numsims=10000, sd=1. \label{fig_hassell_risk_local_disp_scl_1}}
\end{center}
\end{figure}
...
numsims=10000, sd=1. \label{fig_hassell_risk_global_disp_scl_1}}
\end{center}
\end{figure}
...
numsims=10000, sd=0.8. \label{fig_msmith_risk_local_disp_scl_0.8}}
\end{center}
\end{figure}
...
numsims=10000, sd=0.8. \label{fig_msmith_risk_global_disp_scl_0.8}}
\end{center}
\end{figure}
...
numsims=10000, sd=1. \label{fig_risk_local_disp_pennycuick_r_vary_scl_1}}
\end{center}
\end{figure}
...
numsims=10000, sd=1. \label{fig_risk_global_disp_pennycuick_r_vary_scl_1}}
\end{center}
\end{figure}
...
numsims=10000, sd=0.6. \label{fig_risk_local_disp_verhulst_r_vary_scl_0.6}}
\end{center}
\end{figure}
...
numsims=10000, sd=0.6. \label{fig_risk_global_disp_verhulst_r_vary_scl_0.6}}
\end{center}
\end{figure}
...
numsims=10000, sd=0.6. \label{fig_risk_local_disp_abrewer_r_vary_scl_0.6}}
\end{center}
\end{figure}
...
numsims=10000, sd=0.6. \label{fig_risk_global_disp_abrewer_r_vary_scl_0.6}}
\end{center}
\end{figure}
