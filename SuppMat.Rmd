---
title: "Supporting Information : Asymmetric tail-dependence in extreme climatic events affects population-extinction risk"
...
pdf("./Results/fig_4_copula.pdf", height=3, width=12)
op<-par(mfrow=c(1,4),mar=c(4,4,2,2),mgp=c(2,1,0))

# extreme left tail
plot(xtc_l[,1],xtc_l[,2],col=rgb(0,0,0,0.05),pch=19,xlab="u",ylab="v",cex.lab=2)
legend(x="topleft",legend="A",bty="n",cex=2)

# moderate left tail
plot(mtc_l[,1],mtc_l[,2],col=rgb(0,0,0,0.05),pch=19,xlab="u",ylab="v",cex.lab=2)
legend(x="topleft",legend="B",bty="n",cex=2)

# moderate right tail
plot(mtc_r[,1],mtc_r[,2],col=rgb(0,0,0,0.05),pch=19,xlab="u",ylab="v",cex.lab=2)
legend(x="topleft",legend="C",bty="n",cex=2)

# extreme right tail
plot(xtc_r[,1],xtc_r[,2],col=rgb(0,0,0,0.05),pch=19,xlab="u",ylab="v",cex.lab=2)
legend(x="topleft",legend="D",bty="n",cex=2)
...
\caption[Extinction risk from single patch age structured models with time when vital rates have either left (C) or right tail (SC) dependence.]{Extinction risk (shading represents 95\% confidence interval) from single patch age structured models with time for 10000 simulations. Fecundity of adults and survival of eggs have moderate tail dependence in either left or right tail in their corresponding copula structure. Moderate left tail dependence (or right tail dependence) was generated by a bivariate Clayton (or Survival Clayton) copula with Kendall correlation coefficient = 0.6, $(A)$ for linear model ($\Omega = \infty$) and for $(B)$ non-linear model ($\Omega = 100$).\label{fig_extrisk_agestr_mtd}}
\end{center}
\end{figure}
...
\caption[Analogous figure as that of Fig. \ref{MT-fig_extrisk_allmodel_localdisp_etd} in the maintext but with moderate tail dependence in spatially correlated noises.]{Analogous figure as that of Fig. \ref{MT-fig_extrisk_allmodel_localdisp_etd} in the maintext but with moderate tail dependence in spatially correlated noises.\label{fig_extrisk_allmodel_localdisp_mtd}}
\end{center}
\end{figure}
...
\caption[Analogous figure as that of Fig. \ref{fig_extrisk_allmodel_globaldisp_mtd} but with moderate tail dependence in spatially correlated noises.]{Analogous figure as that of Fig. \ref{fig_extrisk_allmodel_globaldisp_mtd} but with moderate tail dependence in spatially correlated noises.\label{fig_extrisk_allmodel_globaldisp_mtd}}
\end{center}
\end{figure}
...
tail dependent noises; Moderate left tail dependence (or right tail dependence) was generated 
...
# Generating matrix with extreme tail dependence  \label{extr_tail_dep}

For extreme tail-dependence algorithm, a $N \times n$ matrix, M was generated of independent 
...
generating extreme tail dependence matrix is flexible enough, so that 
