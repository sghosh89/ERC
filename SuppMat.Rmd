...
```{r sim_linear_mtd, echo=F,  cache=T, cache.extra=list(seed,mtime("ext_risk_cop_age_str.R"))}
...
```{r sim_nonlinear_mtd, echo=F,  cache=T, cache.extra=list(seed,mtime("ext_risk_cop_age_str.R"))}
...
for(i in c(1:length(rlist))){
    ...
    ss<-varying_d(numsims=numsims,numsteps=numsteps,numlocs=numlocs,p0=K,params=params,ext_thrs=ext_thrs,
                 scl=scl,model=model,disp_everywhere=F,plotteron = F,resloc=resloc, getacf=F)
...
for(r in rlist){
    ...
    params<-c(r,K)
    ss<-varying_d(numsims=numsims,numsteps=numsteps,numlocs=numlocs,p0=K,params=params,ext_thrs=ext_thrs,
                 scl=scl,model=model,disp_everywhere=T,plotteron = F,resloc=resloc, getacf=F)
...
```{r sim_verhulst_scl_0.6, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R"))}
set.seed(seed)
source("mydispersal.R")
...
```{r sim_abrewer_scl_0.6, echo=F,  cache=T, cache.extra=list(seed,mtime("mydispersal.R"),mtime("ExtremeTailDep.R"))}
set.seed(seed)
source("mydispersal.R")
